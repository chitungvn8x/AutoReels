            ...
            try:
                with paths["link_file"].open("r", encoding="utf-8") as f:
                    for line in f:
                        ...
                        # Parse format: Link|Time
                        parts = line.split("|")
                        link = parts[0]
                        scan_time = parts[1] if len(parts) > 1 else ""

                        # [CHECK] Nếu link đã có trong lịch sử tải -> BỎ QUA
    ...
    def get_original_videos(self, cat, sub):
        ...
        for f in files:
            ...
            results.append({
                "path": str(f),
                "name": f.name,
                "status": f"Đã sửa: {edit_time}" if is_edited else "CHƯA SỬA",
    ...
    def batch_process_videos(self, file_paths, edit_settings):
        ...
        for i, input_path_str in enumerate(file_paths):
            try:
                ...
                suffix = "_ed"
                if edit_settings.get("speed") != 1.0: suffix += f"_x{edit_settings['speed']}"
                new_name = f"{input_path.stem}{suffix}.mp4"
