...
class SettingsTab:
    ...
    def setup_ui(self):
        ...
        self.create_crud_buttons(f_cat, "CAT", "Nhập tên Danh mục...")
        ...
        self.create_crud_buttons(f_sub, "SUB", "Nhập tên Ngách...")
        ...
        self.t_cap = ctk.CTkTextbox(self.scroll_content, height=200)
        self.t_cap.pack(fill="both", expand=True, padx=5, pady=(0, 5))
        self.t_cap.insert("1.0", "Mỗi nội dung 1 dòng...")
        self.bind_placeholder(self.t_cap, "Mỗi nội dung 1 dòng...")
    ...
    def bind_placeholder(self, widget, text):
        widget.bind("<FocusIn>", lambda e: widget.delete("1.0", "end") if widget.get("1.0", "end-1c").strip() == text else widget.configure(text_color="white"))
        widget.bind("<FocusOut>", lambda e: widget.insert("1.0", text) if not widget.get("1.0", "end-1c").strip() else widget.configure(text_color="silver"))
    ...
    def save_current_sub_config(self):
        ...
        if not pid or not tok: messagebox.showerror("Lỗi", "Chưa nhập Page ID hoặc Token!"); return
        ...
        if not fomos or not caps: messagebox.showwarning("Cảnh báo", "Nội dung FOMO hoặc Caption đang trống.")
        ...
        if pid and tok and self.detected_page_name:
            ...
            # Unique by ID
            existing_key = None
            for key, val in self.app.settings["saved_fanpages"].items():
                if val["page_id"] == pid: existing_key = key; break

            if existing_key and existing_key != self.detected_page_name:
                del self.app.settings["saved_fanpages"][existing_key]
    ...
    def import_txt(self, type_item):
        ...
        messagebox.showinfo("Import", f"Đã thêm {count_add} mục mới.")
    ...
    # [FIX] Implement load config logic
    def on_select_sub(self, sub):
        ...
        load_tb(self.t_tag, d.get("hashtags", []), "tag1, tag2...")
        load_tb(self.t_cap, d.get("captions", []), "Mỗi nội dung...")
